...
def MGP_kernel(q,rho0,LumpFactor,LumExp,MaxPoints):
        ''' 
        The MGP Kernel
        '''
        ...
        indx    = np.where(q != 0)
        tmpker2 = kertmp.copy()
        tmpker2[indx] = 4*np.pi*sp.erf(q[indx])**2*LumpFactor*np.exp(-q[indx]**2*LumExp)/q[indx]**2/cTF
        indx    = np.where(q == 0)
...
def MGP_kernel_E1(q,rho0,LumpFactor,LumExp,MaxPoints):
        ''' 
        The MGP Kernel
        '''
        #cTF_WT = 2.87123400018819
        ...
        indx    = np.where(q != 0)
        tmpker2 = kertmp.copy()
        tmpker2[indx] = 4*np.pi*sp.erf(q[indx])*LumpFactor*np.exp(-q[indx]**2*LumExp)/q[indx]**2/cTF
        indx    = np.where(q == 0)
        tmpker2[indx] = q[indx]**2
        tmpker3 = 1.2*LindhardFunction(q/tkf,1.0,1.0) 
